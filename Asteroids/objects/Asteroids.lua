...
function Asteroids(x, y, ast_size, level, sfx)
  ...
  return {
    ...
    destroy = function (self, asteroid_tbl, index, game)
      local MIN_AST_SIZE = math.ceil(ASTEROID_SIZE / 8)

      if self.radius > MIN_AST_SIZE then
        table.insert(asteroid_tbl, Asteroids(self.x, self.y, self.radius, game.level, sfx))
        table.insert(asteroid_tbl, Asteroids(self.x, self.y, self.radius, game.level, sfx))
      end

      if self.radius >= ASTEROID_SIZE / 2 then
        game.score = game.score + 20
      elseif self.radius <= MIN_AST_SIZE then
